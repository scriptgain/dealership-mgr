<x-layouts.shop :title="$page->title">
    ...
    <section class="border-b border-shop-line bg-slate-50">
        <div class="{{ $maxWidth }} mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
            ...
            <p class="mt-2 text-sm text-shop-muted">Last Updated {{ $page->updated_at?->format(config('dealership.date_format', 'M j, Y')) }}</p>
        </div>
    </section>
        ...
        @if (config('dealership.store_email'))
            <div class="mt-10 rounded-2xl bg-slate-50 p-5 ring-1 ring-shop-line">
                <p class="text-sm text-shop-muted">Questions about this policy? Email us at
                    <a href="mailto:{{ config('dealership.store_email') }}" class="font-medium text-brand-700 hover:text-brand-800">{{ config('dealership.store_email') }}</a>.
                </p>
            </div>
        @endif
    </section>
