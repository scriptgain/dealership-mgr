<x-layouts.app :title="$workOrder->number">
    <x-page-header
        ...
        <x-slot:meta>
            ...
            @if ($workOrder->scheduled_at)
                <x-badge color="neutral">{{ $workOrder->scheduled_at->format(config('dealership.date_format', 'M j, Y').' '.config('dealership.time_format', 'g:i A')) }}</x-badge>
            @endif
        </x-slot:meta>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="min-w-0 space-y-4 lg:col-span-2">
            ...
            <x-card title="Parts" subtitle="Issuing a part puts it on the ticket and takes it off the shelf in one step." flush>
                @php($partLines = $workOrder->items->filter->isPart())
                @if ($partLines->isEmpty())
                    <p class="px-5 py-6 text-center text-sm text-slate-500">No parts on this job.</p>
                @else
                    <x-table flush>
                        <thead>
                            <tr><th>Part</th><th class="text-right">Qty</th><th class="text-right">Price</th><th class="text-right">Total</th><th></th></tr>
                        </thead>
                        <tbody>
                            @foreach ($partLines as $item)
                                <tr>
                                    <td>
                                        @if ($item->part)
                                            <a href="{{ route('parts.show', $item->part) }}" class="font-medium text-slate-900 hover:text-brand-700">{{ $item->name }}</a>
                                        @else
                                            <span class="font-medium text-slate-900">{{ $item->name }}</span>
                                        @endif
                                        @if ($item->description)<span class="block text-xs text-slate-400">{{ $item->description }}</span>@endif
                                    </td>
                                    <td class="tabular text-right">{{ $item->quantity }}</td>
                                    <td class="tabular text-right text-slate-500">{{ $item->unit_price_formatted }}</td>
                                    <td class="text-right font-semibold text-slate-900">{{ $item->total_formatted }}</td>
                                    <td class="text-right">
                                        @if (! in_array($workOrder->status, ['completed', 'cancelled'], true))
                                            <form method="POST" action="{{ route('work-orders.parts.return', [$workOrder, $item]) }}">
                                                @csrf @method('DELETE')
                                                <x-button type="submit" size="sm" variant="ghost">Return To Stock</x-button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
                ...
                @if (! in_array($workOrder->status, ['completed', 'cancelled'], true))
                    <form method="POST" action="{{ route('work-orders.parts.issue', $workOrder) }}"
                          class="flex flex-wrap items-end gap-3 border-t border-slate-200 px-5 py-4">
                        @csrf
                        <x-field label="Part" class="min-w-56 flex-1" :error="$errors->first('part_id')">
                            <x-select name="part_id" required>
                                <option value="">Choose a part</option>
                                @foreach ($parts as $part)
                                    <option value="{{ $part->id }}">{{ $part->part_number }} - {{ $part->name }} ({{ $part->onHand() }} on hand)</option>
                                @endforeach
                            </x-select>
                        </x-field>
                        <x-field label="Qty" class="w-24">
                            <x-input type="number" min="1" name="quantity" value="1" required />
                        </x-field>
                        <x-field label="Price (cents)" class="w-36" hint="Blank uses the shelf price.">
                            <x-input type="number" min="0" name="unit_price_cents" />
                        </x-field>
                        <x-button type="submit" size="sm" variant="secondary" icon="plus">Issue To Job</x-button>
                    </form>
                @endif
            </x-card>

            <x-card title="Labour" subtitle="Clock time is what happened; billed time is what the customer pays." flush>
                @if ($workOrder->timeEntries->isEmpty())
                    <p class="px-5 py-6 text-center text-sm text-slate-500">No time logged on this job.</p>
                @else
                    <x-table flush>
                        <thead>
                            <tr><th>Technician</th><th>Activity</th><th>Started</th><th class="text-right">Clock</th><th class="text-right">Billed</th></tr>
                        </thead>
                        <tbody>
                            @foreach ($workOrder->timeEntries as $entry)
                                <tr>
                                    <td class="font-medium text-slate-900">{{ $entry->user?->name }}</td>
                                    <td class="text-slate-600">{{ $entry->activityLabel() }}</td>
                                    <td class="text-slate-500">{{ $entry->started_at->format('j M, H:i') }}</td>
                                    <td class="tabular text-right text-slate-600">
                                        {{ number_format($entry->elapsedMinutes() / 60, 2) }} hr
                                        @if ($entry->isOpen())<span class="block text-xs font-semibold text-emerald-700">running</span>@endif
                                    </td>
                                    <td class="tabular text-right font-semibold text-slate-900">{{ $entry->hoursLabel() }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </x-table>
                    <div class="flex items-center justify-between border-t border-slate-200 px-5 py-3.5 sm:px-6">
                        <span class="text-sm font-medium text-slate-900">Total Billed Labour</span>
                        <span class="tabular text-base font-semibold text-slate-900">{{ number_format($workOrder->billedHundredths() / 100, 2) }} hr</span>
                    </div>
                @endif
            </x-card>
            ...
            <x-card title="Schedule">
                <dl class="space-y-3 text-sm">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <dd class="text-slate-900">{{ $workOrder->scheduled_at?->format(config('dealership.date_format', 'M j, Y').' '.config('dealership.time_format', 'g:i A')) ?? 'Not scheduled' }}</dd>
                    </div>
                    ...
                    @if ($workOrder->completed_at)
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <dd class="text-slate-900">{{ $workOrder->completed_at->format(config('dealership.date_format', 'M j, Y')) }}</dd>
                        </div>
                    @endif
                </dl>
            </x-card>
