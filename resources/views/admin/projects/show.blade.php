                        ...
                        <tbody>
                            @foreach ($project->workOrders as $workOrder)
                                <tr>
                                    ...
                                    <td class="text-slate-600">{{ $workOrder->scheduled_at?->format(config('dealership.date_format', 'M j, Y')) ?? 'Not scheduled' }}</td>
            ...
            <x-card title="Timeline">
                <dl class="space-y-3 text-sm">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <dd class="text-slate-900">{{ $project->starts_on?->format(config('dealership.date_format', 'M j, Y')) ?? 'Not set' }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <dt class="text-slate-500">Due</dt>
                        <dd class="text-slate-900">{{ $project->due_on?->format(config('dealership.date_format', 'M j, Y')) ?? 'Not set' }}</dd>
                    </div>
                    @if ($project->completed_at)
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-slate-500">Completed</dt>
                            <dd class="text-slate-900">{{ $project->completed_at->format(config('dealership.date_format', 'M j, Y')) }}</dd>
                        </div>
                    @endif
                    <div class="flex items-center justify-between gap-3">
