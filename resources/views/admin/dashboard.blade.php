<x-layouts.app title="Dashboard">
    ...
    <section class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200" aria-labelledby="revenue-heading">
        <div class="grid gap-6 p-5 sm:p-6 lg:grid-cols-5 lg:items-center lg:gap-8">
            ...
            {{-- Sparkline. Registered by public/js/dealership-admin.js. --}}
            ...
            <x-card flush>
                @if ($lowStock->isEmpty())
                    <x-empty-state icon="box" title="Stock Levels Look Healthy"
                        description="A variant shows up here once it drops to {{ config('dealership.low_stock_threshold', 5) }} or fewer in stock. Only variants with inventory tracking switched on are counted." />
                @else
                    <x-table flush>
                        <thead>
                            <tr>
