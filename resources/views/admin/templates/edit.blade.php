        ...
        <x-slot:actions>
            ...
            @if ($override)
                <x-confirm-action
                    ...
                    :message="'Your customised version of ' . $meta['label'] . ' will stop being used immediately and the template DealershipMGR ships will take over. Your edit history is kept, so you can still bring any earlier version back afterwards.'"
                        ...
                        <tbody>
                            @foreach ($versions as $version)
                                <tr class="vx-rail vx-rail-{{ $version->action === 'reset' ? 'danger' : 'none' }}">
                                    ...
                                    <td class="text-slate-500">{{ $version->created_at?->format(config('dealership.date_format').' '.config('dealership.time_format')) }}</td>
