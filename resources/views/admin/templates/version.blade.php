<x-layouts.app :title="'Version #' . $version->id">
    <x-page-header
        ...
        <x-slot:meta>
            ...
            <span class="text-xs text-slate-500">{{ $version->user?->name ?? 'System' }} &middot; {{ $version->created_at?->format(config('dealership.date_format').' '.config('dealership.time_format')) }}</span>
