...
@php
    $maxWidth = $maxWidth ?? config('dealership.max_width', 'max-w-6xl');
@endphp
<!DOCTYPE html>
    ...
    <div class="bg-chrome text-slate-300 text-xs sm:text-sm">
        <div class="{{ $maxWidth }} mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex h-10 items-center justify-between gap-4">
                <p class="truncate text-slate-400">{{ config('dealership.store_tagline') }}</p>
            ...
            <div class="flex h-16 items-center justify-between gap-4">
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    <nav class="hidden lg:flex items-center gap-1 ml-4">
                        @foreach ([['Home', 'shop.home', 'home'], ['Inventory', 'shop.inventory', 'car'], ['Request Service', 'shop.request', 'plus'], ['Help', 'shop.help', 'info']] as [$navLabel, $navRoute, $navIcon])
                            @php($active = request()->routeIs($navRoute)
                                || ($navRoute === 'shop.help' && request()->routeIs('shop.help.*'))
                                || ($navRoute === 'shop.inventory' && request()->routeIs('shop.vehicle')))
        ...
        <div class="{{ $maxWidth }} mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 gap-10 lg:grid-cols-12">
                ...
                <div class="min-w-0 lg:col-span-6">
                    ...
                    <p class="mt-3 max-w-sm text-sm leading-relaxed text-shop-muted">{{ config('dealership.store_tagline') }}</p>
                ...
                <div class="min-w-0 lg:col-span-2">
                    ...
                    <ul class="mt-4 space-y-2.5 text-sm text-shop-muted">
                        ...
                        @if (config('dealership.store_email'))
                            <li><a href="mailto:{{ config('dealership.store_email') }}" class="hover:text-shop-ink transition">Contact Us</a></li>
                        @endif
                    </ul>
                </div>
