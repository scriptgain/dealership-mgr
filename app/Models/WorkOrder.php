...
class WorkOrder extends Model
{
    ...
    /** Clock hours actually spent on this job, across every technician. */
    public function clockedHundredths(): int
    {
        return (int) $this->timeEntries()->whereNotNull('minutes')->get()
            ->sum(fn (TimeEntry $e) => (int) round($e->minutes / 60 * 100));
    }

    /** What the customer is charged for, which is not the same figure. */
    public function billedHundredths(): int
    {
        return (int) $this->timeEntries()->sum('billed_hundredths');
    }
