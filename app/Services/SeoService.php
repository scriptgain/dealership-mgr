 ...
 *
 * 1. VARIANTS. DealershipMGR picks variants client-side, so a variant is not its own
...
class SeoService
{
    ...
    {
        $store = (string) config('dealership.store_name');

        return $this->resolved = $this->finish(new SeoData(
            title: $store,
            description: $this->clean(config('seo.default_description') ?: config('dealership.store_tagline')),
    ...
    public function product(Product $product): SeoData
    {
        ...
        $og = array_filter([
            ...
            'product:price:currency' => config('dealership.currency', 'USD'),
    ...
    public function catalog(?LengthAwarePaginator $products = null): SeoData
    {
        ...
        return $this->resolved = $this->finish(new SeoData(
            title: 'All Products',
            description: $this->clean('Browse every product available from '.config('dealership.store_name').'.'),
    ...
    public function collectionsIndex(): SeoData
    {
        return $this->resolved = $this->finish(new SeoData(
            title: 'Collections',
            description: $this->clean('Browse '.config('dealership.store_name').' by collection.'),
    ...
    public function search(?string $term): SeoData
    {
        ...
        return $this->resolved = $this->finish(new SeoData(
            title: $title,
            description: $this->clean('Search '.config('dealership.store_name').'.'),
    ...
    public function resolve(?string $fallbackTitle = null): SeoData
    {
        ...
        if ($this->isPrivateRoute()) {
            return $this->transactional($fallbackTitle ?: (string) config('dealership.store_name'));
        }

        return $this->resolved = $this->finish(new SeoData(
            title: $fallbackTitle ?: (string) config('dealership.store_name'),
            description: $this->clean(config('seo.default_description') ?: config('dealership.store_tagline')),
            canonical: $this->urls->current(),
            robots: $this->robots(),
            og: ['og:type' => 'website'],
        ), rawTitle: $fallbackTitle ?: (string) config('dealership.store_name'));
    }
    ...
    public function autoDescription(Product|Collection $entity): string
    {
        ...
        return $this->clean(
            'Shop the '.$entity->name.' collection at '.config('dealership.store_name').'.',
    ...
    private function productSentence(Product $product): string
    {
        ...
        return $sentence.' Available now at '.config('dealership.store_name').'.';
    }
    ...
    private function finish(SeoData $data, string $rawTitle, bool $templated = true): SeoData
    {
        ...
        // name to it produces "Waxed Cotton Overnight Bag: DealershipMGR: DealershipMGR"
        ...
        $data->og = array_filter(array_merge([
            'og:site_name' => config('dealership.store_name'),
            'og:title' => $rawTitle,
