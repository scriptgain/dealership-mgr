...
class OrderPayments
{
    ...
    */
    ...
    private static function createIntent(Order $order, int $amountCents): array
    {
        $params = [
            'amount' => $amountCents,
            'currency' => strtolower($order->currency ?: config('dealership.currency', 'USD')),
            'description' => Str::limit(config('dealership.store_name', 'Order').' '.$order->number, 200, ''),
