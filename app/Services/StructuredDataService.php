 ...
 * DealershipMGR ships no reviews table, so on a stock install those nodes are simply
...
class StructuredDataService
{
    ...
    private function offers(Product $product): ?array
    {
        ...
        return [
            '@type' => 'AggregateOffer',
            'priceCurrency' => config('dealership.currency', 'USD'),
    ...
    {
        if ($inStock) {
            return $tracked && config('dealership.allow_backorder')
    ...
    /**
     * DealershipMGR has no reviews table today. This stays here (rather than being
    ...
    public function organization(): array
    {
        ...
        return array_filter([
            ...
            'name' => config('seo.organization_name') ?: config('dealership.store_name'),
            'url' => $this->urls->home(),
            'logo' => config('seo.organization_logo') ?: null,
            'email' => config('dealership.store_email') ?: null,
            'telephone' => config('dealership.store_phone') ?: null,
            'sameAs' => $sameAs ?: null,
        ], fn ($value) => $value !== null && $value !== '' && $value !== []);
    }
    ...
    public function website(): array
    {
        return [
            ...
            'name' => config('dealership.store_name'),
            'url' => $this->urls->home(),
