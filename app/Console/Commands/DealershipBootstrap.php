 ...
 * (BOOTSTRAP_CMD="dealership:bootstrap" in products/dealership-mgr.env).
 *
 * Everything here is idempotent — the installer is safe to re-run, and so is
 * this command.
 */
class DealershipBootstrap extends Command
{
    protected $signature = 'dealership:bootstrap {--fresh-token : Issue a new full-access token even if one exists}';
    ...
    public function handle(): int
    {
        ...
        $this->newLine();
        $this->info('DealershipMGR is bootstrapped. Finish at /setup, then add products at /admin/products.');
    ...
    private function seedSettings(): void
    {
        $defaults = [
            ...
            'store_tagline' => config('dealership.store_tagline'),
            'currency' => config('dealership.currency'),
            'currency_symbol' => config('dealership.currency_symbol'),
            'currency_decimals' => (string) config('dealership.currency_decimals'),
            'products_per_page' => (string) config('dealership.products_per_page'),
            'low_stock_threshold' => (string) config('dealership.low_stock_threshold'),
            'guest_checkout' => '1',
            'terms_required' => '1',
            'order_prefix' => config('dealership.order_prefix'),
            'tax_mode' => config('dealership.tax_mode'),
            'default_gateway' => 'manual',
            'stripe_mode' => 'test',
            'max_width' => config('dealership.max_width'),
            'rows_per_page' => '25',
        ];
    ...
    private function issueToken(): void
    {
        ...
        if (! $user) {
            $this->warn('No staff user yet; finish /setup, then re-run `dealership:bootstrap` to issue the API token.');
